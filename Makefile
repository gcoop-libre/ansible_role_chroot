...
GIT_HTTP      ?= $$(git config --get remote.origin.url | grep http | rev | cut -d/ -f2- | rev | sed 's/:\//\//g' | sed 's/https\/\//https:\/\//g')
...
tools:
	[[ -d $(TOOLS_DIR) ]] || git clone $(GIT_URL)/$(TOOLS) $(TOOLS_DIR)
...
syntax: inventory_env requirements
